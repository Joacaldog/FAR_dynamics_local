...
parser.add_argument("-r", "--receptor", type=str,
                    help="absolute path of receptor file in PDB format or folder containing multiple receptors", required=True)
...
parser.add_argument("-in", "--in_folder", type=str,
                    help="Specify folder absolute path where all config files are located (e.g: /home/user/FAR_and_prod_in_files)", required=True)
parser.add_argument("-t", "--threads", type=int,
                    help="Number of threads to utilize", required=True)
parser.add_argument("-co", "--cofactor", nargs='?',
                    help="Optional: Absolute path of cofactor file in SDF format or folder containing multiple cofactors (if folder RECEPTOR'S FILENAME WITHOUT EXTENSION MUST BE WITHIN COFACTOR'S FILE NAME)")
parser.add_argument("-cop", "--cofactor_prefix", nargs='?',
                    help="if you provided a cofactor's folder you must enter a prefix that will be used to find the file (e.g.: -cop SAM ---will-find--> SAM_receptorName.sdf)")
...
if cofactor_folder:
    if os.path.isdir(receptor) and prefix_cofactor == None:
...
if __name__ == '__main__':
    ...
    if cofactor_folder != None:
        session_name = f'{initial_path}/run_{ligand.replace(".sdf", "")}_wCofactor'
    ...
    if os.path.isdir(receptor):
        ...
        os.system(f'cp {receptor} {initial_path}')
        receptor_list = [receptor.split("/")[-1]]
        receptor_list = [f'{initial_path}/{receptor_file}' for receptor_file in receptor_list]
        with multiprocessing.Pool(processes=threads) as pool:
            pool.map(prepare_receptor, receptor_list)
        gpu_cycle = itertools.cycle(gpu_ids)
        with multiprocessing.Pool(processes=threads) as pool:
