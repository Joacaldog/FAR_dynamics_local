...
parser.add_argument("-in", "--in_folder", type=str,
                    help="Specify folder path where all '.in' files are located (e.g: /home/user/FAR_and_prod_in_files)", required=True)
...
def run_dynamics(i):
    ...
    os.system(f"pdb4amber og_receptor.pdb > receptor_mod.pdb")
    cp_in_files = f"cp {in_folder}/* ."
    os.system(cp_in_files)
    charge = charge_check()
    print(f"Net charge of peptide is {charge}")
    cmd_mol2_to_prepi = f"antechamber -i pep.sdf -fi sdf -o pep.mol2 -fo mol2 -c bcc -nc {charge} -pf Y> antechamber.log"
    ...
    tmux_init = f"tmux new-session -d -s {rep_folder}"
    os.system(tmux_init)
    os.system(f'tmux send-keys -t {rep_folder} "sleep 2" C-m')
    os.system(f'tmux send-keys -t {rep_folder} "source {in_folder}/amber.sh" C-m')
    tmux_send_gpu = f'tmux send-keys -t {rep_folder} "export CUDA_VISIBLE_DEVICES={gpu_num}" C-m'
    os.system(tmux_send_gpu)
    tmux_send_run = f'tmux send-keys -t {rep_folder} "python3 run_commands.py FAR_commands.txt" C-m'
    os.system(tmux_send_run)
    convert_trj3 = f'tmux send-keys -t {rep_folder} "cpptraj -p complex_solvated.prmtop -y min3.rst -x min3.trj" C-m'
    os.system(convert_trj3)
    extrac_coords_cmd = f'tmux send-keys -t {rep_folder} "$AMBERHOME/bin/mm_pbsa.pl extract_coords_mod.mmpbsa 1> extract_coords.log 2>extract_coords.err" C-m'
    os.system(extrac_coords_cmd)
    bindingE_cmd = f'tmux send-keys -t {rep_folder} "$AMBERHOME/bin/mm_pbsa.pl binding_energy.mmpbsa > binding_energy.log" C-m'
    os.system(bindingE_cmd)
    os.system(f'tmux send-keys -t {rep_folder} "mv snapshot* FAR_results" C-m')
    os.system(f'tmux send-keys -t {rep_folder} "mv coords extract_coords.err extract_coords.log binding_energy.log FAR_results; mkdir coords" C-m')
    #Here starts prod commands
    if prod == True:
        tmux_send_run2 = f'tmux send-keys -t {rep_folder} "python3 run_commands.py commands.txt" C-m'
        os.system(tmux_send_run2)
        convert_trj = f'tmux send-keys -t {rep_folder} "cpptraj -p complex_solvated.prmtop -y prod.mdcrd -x prod.trj" C-m'
        os.system(convert_trj)
        remove_wat_cmd = f'tmux send-keys -t {rep_folder} "cpptraj -i remove_water_prod_mdcrd_mod.in" C-m'
        os.system(remove_wat_cmd)
        os.system(f'tmux send-keys -t {rep_folder} "cpptraj -i measure_prod_rmsd.in" C-m')
        os.system(f'tmux send-keys -t {rep_folder} "cpptraj -i measure_prod_rmsf.in" C-m')
        extrac_coords_cmd = f'tmux send-keys -t {rep_folder} "$AMBERHOME/bin/mm_pbsa.pl extract_coords_prod_mod.mmpbsa 1> extract_coords_prod.log 2>extract_coords_prod.err" C-m'
        os.system(extrac_coords_cmd)
        bindingE_cmd = f'tmux send-keys -t {rep_folder} "$AMBERHOME/bin/mm_pbsa.pl binding_energy.mmpbsa > binding_energy_prod.log" C-m'
        os.system(bindingE_cmd)
        # flag_cmd = f'tmux send-keys -t "{rep_folder}:0" "> flag && exit" C-m;'
        # os.system(flag_cmd)
        # flag_check = 'until [ -f "flag" ]; do sleep 5; done'
        # os.system(flag_check)
    print("finished")
